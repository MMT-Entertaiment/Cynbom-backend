...
module.exports = {
  ...
  modifierEpisode(titreSerie, saison, numero, video) {
    const serie = db.prepare('SELECT id FROM series WHERE titre = ?').get(titreSerie);
    if (!serie) return false;
    const result = db.prepare('UPDATE episodes SET video = ? WHERE serie_id = ? AND saison = ? AND numero = ?').run(video, serie.id, saison, numero);
    return result.changes > 0;
  },
