...
// GET série par url_id
app.get('/serie/:url_id', (req, res) => {
  const serie = db.getSerieByUrlId(req.params.url_id);
...
app.patch('/series/:titre/episodes/:saison/:numero', (req, res) => {
  ...
  const ok = db.modifierEpisode(req.params.titre, req.params.saison, req.params.numero, video);
  res.json({ success: ok });
});
